...
public class OddNumberMultiplicationStream {
    ...
    public static void main(String[] args) {
        ...
        properties.setProperty(StreamsConfig.APPLICATION_ID_CONFIG, "streams-assignment-flow");
        ...
        KStream<String, Integer> stream = builder.stream("kafka-streams-assignment-input-topic");
        stream.foreach((k, v) -> LOGGER.info("The key: {}, value: {}", k, v));
        stream.filter((k, v) -> v % 2 != 0)
                .map((k, v) -> KeyValue.pair(String.valueOf(v), Math.multiplyExact(v, 3)))
                .to("kafka-streams-assignment-output-topic");
